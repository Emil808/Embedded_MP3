...
void lcd_update_playstatus(bool playIn) {
  lcd_write_command(0x80);     // set X address to 0
  lcd_write_command(0x40 | 5); // set Y address to bank 5

  lcd_putchar(' ');
  lcd_putchar_playstatus(playIn);
}

void lcd_update_volume(int vol) {
  lcd_write_command(0x80 | 66); // set X address to 0
  lcd_write_command(0x40 | 5);  // set Y address to bank 5
